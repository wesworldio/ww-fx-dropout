...
class MetalRenderer: NSObject, MTKViewDelegate {
    ...
    private func setupPipeline(view: MTKView) {
        // Load shader source from file system or use embedded source
        ...
        for path in possiblePaths {
            if fileManager.fileExists(atPath: path) {
                if let source = try? String(contentsOfFile: path, encoding: .utf8) {
                    shaderSource = source
                    print("✓ Loaded shaders from: \(path)")
                    break
                }
            }
        }
        ...
        guard let shaderSource = shaderSource else {
            fatalError("Failed to load shader source file from: \(possiblePaths.joined(separator: ", "))")
        }
